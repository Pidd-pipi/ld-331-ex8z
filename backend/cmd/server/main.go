...
func main() {
	...
	ruleEngine := service.NewRuleEngine(schedRepo, ruleRepo, holidayRepo)
	schedSvc := service.NewScheduleService(schedRepo, staffRepo, shiftRepo, ruleRepo, holidayRepo, ruleEngine, logger)
	reqSvc := service.NewShiftRequestService(reqRepo, schedRepo, shiftRepo, ruleEngine, db, logger)
