...
- **规则排班**：配置连续工作天数、周末轮循、节假日优先、夜班限制；一键生成指定日期范围的班表。自动生成同时遵守连续工作上限、夜班后不得排白班和节假日优先；实在无法回避的冲突（如单人科室连逢多个节假日）会以 `has_conflict` 与中文 `conflict_reasons` 记录在对应班表行。
- **可视化班表**：按科室、人员、冲突状态与日期筛选，白班/中班/夜班/休息采用不同颜色，冲突行标红并悬浮显示原因；下拉调整即为手动微调。手动调整与调班审批通过前都会按最新规则重新复核，任一冲突都会整次拒绝（HTTP 409），原班表和申请状态保持不变。
