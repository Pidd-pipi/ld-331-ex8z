...
- **排班冲突闭环**：自动生成同时遵守连续工作上限、夜班后不得排白班与节假日优先，无法避免的冲突（如最小在岗人数）会写入该人员班表的 `conflict_reason` 字段；手动调整与调班审批均按最新规则在事务前复查，存在冲突时整次拒绝（HTTP 409 / `40901`），原班表与申请状态保持不变。排班页直接展示冲突原因，并支持“仅看冲突”过滤，刷新或重新筛选后仍可回读。
- **调班与替班**：员工提交双方班次的调换申请，主管审批时在一个数据库事务中交换排班，记录申请人、审批人与状态；审批触发规则复查，冲突即拒绝。
